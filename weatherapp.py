...
INFO: You need to create a account for an API key here: https://opencagedata.com/api#quickstart
"""
...
COORDS_KEY = 'YOUR API KEY' # <-- Create a free account for your API key: https://opencagedata.com/api#quickstart
COORDS_JSON = (
...
def get_weather(place:str) -> dict:
    ...
    with urlopen(COORDS_API.format(place.encode(), COORDS_KEY)) as data:
        ...
        addr = (meta.get('postcode') if meta.get('postcode') is not None else '', 
            meta.get('town') if meta.get('town') is not None else meta.get('city'), meta.get('country')
        )
